...
class InternshipDetailView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(internship.jobRoles?.first ?? 'Internship Detail'),
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text("${internship.numberOfOpenings ?? 0}"),

            const SizedBox(height: 20),

            SectionTitle(title: "Online Test Date"),
            Text(_formatDate(internship.onlineTestDate)),
            ...
            Wrap(
              spacing: 8,
              children:
                  internship.skills?.map((e) => CustomChip(text: e)).toList() ??
                  [],
            ),
            ...
            Text(internship.eligibilityCriteria ?? ''),
          ],
        ),
      ),
    );
