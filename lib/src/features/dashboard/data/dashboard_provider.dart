...
void clearFilters() {
  ...
  if (!matches) return false;
    ...
    if (paidOnly &&
        (job.packageDetails?.totalCTC ?? 0) <= 0) {
      return false;
    }
...
List<JobModel> _sortJobs(List<JobModel> jobs) {
  switch (_sortOption) {
    case SortOption.newestFirst:
      jobs.sort((a, b) =>
          (b.offerRolloutDate ?? DateTime(2000))
              .compareTo(a.offerRolloutDate ?? DateTime(2000)));
      break;

    case SortOption.oldestFirst:
      jobs.sort((a, b) =>
          (a.offerRolloutDate ?? DateTime(2000))
              .compareTo(b.offerRolloutDate ?? DateTime(2000)));
      break;
    ...
    case SortOption.highestStipend:
      jobs.sort((a, b) =>
          (b.packageDetails?.totalCTC ?? 0)
              .compareTo(a.packageDetails?.totalCTC ?? 0));
      break;

    case SortOption.nearestDeadline:
      jobs.sort((a, b) =>
          (a.onlineTestDate ?? DateTime(2100))
              .compareTo(b.onlineTestDate ?? DateTime(2100)));
      break;
  }
...
List<InternshipModel> applyInternshipFilters(
  ...
  if (!matches) return false;
    ...
    if (paidOnly &&
        (internship.packageDetails?.totalCTC ?? 0) <= 0) {
      return false;
    }
...
List<InternshipModel> _sortInternships(
    List<InternshipModel> internships) {
    ...
    case SortOption.newestFirst:
      internships.sort((a, b) =>
          (b.onlineTestDate ?? DateTime(2000))
              .compareTo(a.onlineTestDate ?? DateTime(2000)));
      break;

    case SortOption.oldestFirst:
      internships.sort((a, b) =>
          (a.onlineTestDate ?? DateTime(2000))
              .compareTo(b.onlineTestDate ?? DateTime(2000)));
      break;

    case SortOption.companyNameAZ:
      internships.sort((a, b) =>
          (a.companyPosted?.companyDetails?.companyName ?? "")
              .compareTo(
          b.companyPosted?.companyDetails?.companyName ?? ""));
      break;
